...
class Experiment():
	def __init__(self, name, root):
		...
		self.best_weights = os.path.join(self.weights_dir, 'best_weights.pth')
		self.best_optimizer = os.path.join(self.weights_dir, 'best_optim.pth')
	...
	def save_weights(self, model, trn_loss, val_loss, trn_err, val_err):
		...
		if self.is_best_loss(val_loss):
			...
			shutil.copyfile(weights_fpath, self.best_weights)
	...
	def save_optimizer(self, optimizer, val_loss):
		...
		if self.is_best_loss(val_loss):
			...
			shutil.copyfile(optim_fpath, self.best_optimizer)
