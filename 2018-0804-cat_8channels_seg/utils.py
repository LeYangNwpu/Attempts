...
def train(model, trn_loader, optimizer, criterion, epoch):
	...
	trn_loss = 0
	trn_error = 0
	for batch_idx, (img, targets, img_cont, img_box, cont_box) in enumerate(trn_loader):
...
def test(model, test_loader, criterion, epoch=1):
	...
	test_loss = 0
	test_error = 0
	for img, target, img_cont, img_box, cont_box in test_loader:
		inputs = torch.cat((img, img_box, img_cont, cont_box), 1)
		inputs = Variable(inputs.cuda())
...
def file_copy(fol_name):
	...
	shutil.copyfile(ori_boxC_rp + sel_name, res_boxC_rp + sel_name)
