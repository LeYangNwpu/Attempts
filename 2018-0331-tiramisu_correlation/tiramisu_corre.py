...
class ContFilter(nn.Module):
    def __init__(self):
        super(ContFilter, self).__init__()
        self.conv = nn.Conv2d(
            in_channels=3, out_channels=24, kernel_size=3, padding=1, stride=1, bias=True)
        self.relu = nn.ReLU()
        self.norm = nn.BatchNorm2d(num_features=24)
...
cont_filter = ContFilter()
cont_filter = cont_filter.cuda()
...
img_conv = ImgConv()
img_conv = img_conv.cuda()


class FCDenseNet(nn.Module):
	def __init__(self, in_channels=3, down_blocks=(5, 5, 5, 5, 5),
				 up_blocks=(5, 5, 5, 5, 5), bottleneck_layers=5,
				 growth_rate=16, out_chans_first_conv=49, n_classes=12):
	...
	def forward(self, img, cont, box):
		batch, _, _, _ = img.size()
		score_map = torch.cuda.FloatTensor(batch, 1, 128, 128)
		for ibat in range(batch):
			cont_t = cont[ibat, :, :, :]
			cont_t = torch.unsqueeze(cont_t, 0)
			img_t = img[ibat, :, :, :]
			img_t = torch.unsqueeze(img_t, 0)
			filter_map = F.conv2d(cont_t, img_t)
			score_map[ibat, :, :, :] = filter_map.data
		# weight map is the linear normalize the score map
		weight_map = (score_map - score_map.min()) / (score_map.max() - score_map.min())
		weight_map_e = weight_map.expand(-1, 24, 128, 128)
		weight_map_e = Variable(weight_map_e)
		# context feature map, N * 48 * 64 * 64
		cont_map = cont_filter(cont)
		cont_map_wei = cont_map * weight_map_e

		img_filter = img_conv(img)
		out = torch.cat((img_filter, cont_map_wei, box), 1)
		# x = img
		# out = self.firstconv(inputs)
...
def FcDnSubtle(in_channels=3, n_classes=2):
    return FCDenseNet(in_channels=in_channels, down_blocks=(5, 5, 5, 5, 5),
                 up_blocks=(5, 5, 5, 5, 5), bottleneck_layers=5,
                 growth_rate=12, out_chans_first_conv=49, n_classes=n_classes)
