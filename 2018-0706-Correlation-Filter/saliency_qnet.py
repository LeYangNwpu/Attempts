...
class Saliency(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # annot
        path_ann = path_img.replace(self.split, self.split + 'annot').replace('tif', 'png')
        # path_ann = path_ann.replace('tif', 'png')
        target = Image.open(path_ann)
        # box
        path_box = path_img.replace(self.split, self.split + 'box').replace('tif', 'pkl')
        pkl_file = open(path_box, 'rb')
        box = pickle.load(pkl_file)
        ...
        return img, target, np.transpose(box)
...
class TestImage(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img, nlist, np.transpose(box)
