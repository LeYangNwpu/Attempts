...
class Experiment():
	def __init__(self, name, root):
		...
		self.best_weights_savepath = os.path.join(self.weights_dir, 'best_weights.pth')
		self.best_optimizer_savepath = os.path.join(self.weights_dir, 'best_optim.pth')
	...
	def save_weights(self, model, trn_loss, val_loss, trn_err, val_err):
		...
		if val_loss <= self.best_val_loss:
			...
			self.best_val_loss_epoch = self.epoch
			self.best_weights_path = weights_fpath
			shutil.copyfile(self.best_weights_path, self.best_weights_savepath)
	...
	def save_optimizer(self, optimizer, val_loss):
		...
		if val_loss <= self.best_val_loss:
			...
			self.best_val_loss_epoch = self.epoch
			self.best_optimizer_path = optim_fpath
			shutil.copyfile(optim_fpath, self.best_optimizer_savepath)
