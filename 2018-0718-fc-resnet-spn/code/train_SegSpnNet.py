...
import dataset_spn as dataset
import joint_transforms
import experiment
import train_spn_utils as utils
...
def main():
	...
	target_resize = transforms.Compose([joint_transforms.JointResize(112)])
	train_dset = dataset.Saliency(
		args.DATASET_PATH, 'TRain', train_joint_transformer, target_resize=target_resize,
	...
	weight_path = 'pretrain_weights.pth'
	state_pretrain = torch.load(weight_path)
	dict_pretrain = state_pretrain['state_dict']

	model = SegSpnNet.resnet34()
	# # initialize
	# model.apply(utils.weights_init)
	SegSpnNet_dict = model.state_dict()

	pretrained_dict = {k: v for k, v in dict_pretrain.items() if k in SegSpnNet_dict}
	# for k in pretrained_dict:
	# 	print(k)
	SegSpnNet_dict.update(pretrained_dict)
	model.load_state_dict(SegSpnNet_dict)
	...
	exp_dir = args.EXPERIMENT + 'segspn'
	if os.path.exists(exp_dir):
		shutil.rmtree(exp_dir)

	exp = experiment.Experiment('segspn', args.EXPERIMENT)
	exp.init()
